...
import { Route, X, ChevronRight, Clock, MapPin, CornerUpRight, Navigation, Globe, Phone, MessageSquare, Maximize2, Minimize2, Star, Trash2 } from 'lucide-react';
...
export function TripDetails({ trip, allPois, isExpanded, onToggleExpand, onClose, onPoiClick, onRemovePoi }: TripDetailsProps) {
        ...
        <div className="space-y-0">
          ...
          {trip.pois.length === 0 ? (
            <div className="relative pl-10 pb-4">
              ...
              <div className="bg-[#262626] rounded-3xl p-8 text-center border border-white/5 shadow-xl mb-4 mt-2">
                <MapPin size={32} className="mx-auto text-gray-500 mb-3" />
                <h4 className="text-lg font-bold text-gray-300 mb-2">No places added yet</h4>
                <p className="text-gray-500 text-sm mb-6">Start discovering and adding places to your itinerary.</p>
                <button 
                  onClick={() => onPoiClick('swipe')}
                  className="px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl transition-all shadow-lg active:scale-[0.98]"
                >
                  Discover Places
                </button>
              </div>
            ...
            trip.pois.map((poiId, index, arr) => {
              ...
              return (
                <div key={poi.id} className="relative pl-10 pb-4 group">
                  ...
                  <div 
                    className="bg-[#262626] rounded-3xl overflow-hidden border border-white/5 cursor-pointer hover:border-white/20 transition-all shadow-xl mb-4 relative"
                    onClick={() => onPoiClick(poi.id)}
                  >
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        if (window.confirm(`Remove ${poi.name} from this trip?`)) {
                          onRemovePoi(poi.id);
                        }
                      }}
                      className="absolute top-4 right-4 z-10 p-2 bg-black/60 backdrop-blur-md rounded-full text-gray-300 hover:text-red-400 hover:bg-red-500/20 transition-all opacity-0 group-hover:opacity-100 border border-white/10"
